#!/usr/bin/env python3
"""
paper-to-md post-processor
Usage: python postprocess.py <html_dir> <claude_dir> [file.md ...]
  html_dir:   folder with .html/.md files; .vscode/settings.json is built here
  claude_dir: Claude/ subfolder; CSS files are written here
If no files given, processes all *.md in html_dir.
"""
...
def generate_css(claude_dir):
    """Write CSS files into the Claude/ subfolder."""
    out = Path(claude_dir)
    out.mkdir(exist_ok=True)
    (out / 'vscode-markdown.css').write_text(VSCODE_CSS, encoding='utf-8')
    print(f'  [ok] {out / "vscode-markdown.css"}')
    (out / 'obsidian-snippet.css').write_text(OBSIDIAN_CSS, encoding='utf-8')
    print(f'  [ok] {out / "obsidian-snippet.css"}')


def update_vscode_settings(html_dir, claude_dir):
    """Create .vscode/settings.json in html_dir; styles path points to Claude/."""
    claude_name = Path(claude_dir).name          # 'Claude'
    css_rel = f'./{claude_name}/vscode-markdown.css'

    vd = Path(html_dir) / '.vscode'
...
def main():
    ...
    if len(sys.argv) < 3:
        print('Usage: postprocess.py <html_dir> <claude_dir> [file.md ...]')
        print('  html_dir:   folder containing .html and .md files (.vscode/ built here)')
        print('  claude_dir: Claude/ subfolder for CSS (html_dir\\Claude)')
        print('  If no files specified, all *.md in html_dir are processed.')
        sys.exit(1)

    html_dir   = sys.argv[1]
    claude_dir = sys.argv[2]
    files = sys.argv[3:] if len(sys.argv) > 3 else list(Path(html_dir).glob('*.md'))

    print('=== paper-to-md postprocess ===')
    for f in files:
        process_file(str(f))

    generate_css(claude_dir)
    update_vscode_settings(html_dir, claude_dir)

    print('\n=== done ===')
    print('Obsidian white-bg setup (manual):')
    print('  Settings -> Appearance -> CSS snippets -> open folder')
    print(f'  Copy {Path(claude_dir) / "obsidian-snippet.css"} there, then toggle it ON.')
